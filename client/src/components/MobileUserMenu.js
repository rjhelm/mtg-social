...
import { IconButton, Menu, MenuItem, Avatar, ListItemIcon, Divider, } from '@material-ui/core';
...
const MobileUserMenu = ({ user, handleLogout }) => {
    ...
    const loggedUser = storageService.loadUsser() || user;

    return (
        <div>
            {loggedUser ? (
                <IconButton onClick={handleMenu} className={classes.userBtnMob}>
                    {loggedUser?.avatar?.exisets ? (
                        <Avatar
            ...
            <Menu anchorEl={anchorEL}
                ...
                {loggedUser ? (
                    <div>
                        <MenuItem component={RouterLink} to={`/commander/${loggedUser.username}`} onClick={handleClose}>
                            <ListItemIcon>
                                <AccountCircleIcon style={{ marginRight: 7 }} /> Commander Profile
                            </ListItemIcon>
                        </MenuItem>
