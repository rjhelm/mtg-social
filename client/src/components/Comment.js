...
const Comment = ({ comment, user, postReplyId, editComment, deleteComment }) => {
  ...
  const handleCommentEdit = (e) => {
    e.preventDefault();
    editComment(editedCommentBody, comment.id, postReplyId);
    closeInput();
  };
      ...
      {!editOpen ? (
        <div>
          ...
          {user && (user.id === comment.author.id || user.role === 'ADMIN') && (
            <DeleteDialog
              bodyType="comment"
              handleDelete={() => deleteComment(comment.id, postReplyId)}
            />
          )}
        </div>
      ) : (
